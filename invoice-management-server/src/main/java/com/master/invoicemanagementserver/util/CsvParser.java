...
public class CsvParser {
    ...
    public record ParseResult(List<InvoiceRowDTO> validRows, List<ParseError> errors) {}

    public record ParseError(String invoiceId, String message) {}
    ...
    public ParseResult parse(MultipartFile file) {
        ...
        int maxIdx = Math.max(Math.max(idxInvoiceId, idxCustomerName), Math.max(idxAmount, Math.max(idxCurrency, Math.max(idxIssueDate, idxVendorCode))));
        ...
        for (int i = 1; i < rows.size(); i++) {
            ...
            if (row.length <= maxIdx) {
                errors.add(new ParseError("", "Row " + rowNum + ": insufficient columns"));
                continue;
            }
